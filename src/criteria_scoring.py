...
class CriteriaScoring:
    def __init__(self, criteria_config: dict) -> None:
        ...
        self.threshold_m_coast_tile_counting = criteria_config[
            "threshold_m_coast_tile_counting"
        ]
        ...
        if direction == "low_val_high_score":
            ...
            if is_weighted == True:
                if val == low_threshold:
                    ...
                    score = (
                        1.0 - (val - low_threshold) / (high_threshold - low_threshold)
                    ) * weight
                else:
                    score = 0.0
            else:
        ...
        if self.is_weighted == True:
            for col_name in self.scoring_thresholds_dict.keys():
                ...
                (
                    ...
                    direction,
                    weight,
                ) = self.scoring_thresholds_dict[col_name]
                ...
                tile_scores[score_col_name] = tile_scores[col_name].apply(
                    lambda x: self._threshold_score(
                        x,
                        low_threshold,
                        high_threshold,
                        direction,
